...
class LongVMap<V> : MutableMap<Long, V> {
    ...
    class KeyIterator<V>(private val longSparseArray: LongSparseArray<V>) :
        ...
        override fun next(): Long {
            ...
            if (mEntryValid) point++
            mEntryValid = true
            return longSparseArray.keyAt(point)
        }
    ...
    class EntryIterator<V>(private val longSparseArray: LongSparseArray<V>) :
        ...
        override fun next(): MutableMap.MutableEntry<Long, V> {
            ...
            if (mEntryValid) point++
            mEntryValid = true
            val k=longSparseArray.keyAt(point)
            key=k
            return InternalNode(k, longSparseArray)
        }
    ...
    class EntrySet<V>(private val longSparseArray: LongSparseArray<V>) :
        ...
        override fun remove(element: MutableMap.MutableEntry<Long, V>):Boolean {
            val k = element.key
            val index = longSparseArray.indexOfKey(k)
            if (index >= 0) {
                if (longSparseArray.valueAt(index) == element.value) {
        ...
        override fun contains(element: MutableMap.MutableEntry<Long, V>): Boolean {
            /*val v = longSparseArray.get(element.key)
            if (v == null && element.value != null) {
                if (longSparseArray.get(element.key, element.value) === element.value) {
                    return false
                }
            }
            if (v == element.value) return true
            return longSparseArray.containsKey(element.key)*/
            val index = longSparseArray.indexOfKey(element.key)
